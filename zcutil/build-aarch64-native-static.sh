...
HOST="$HOST" BUILD="$BUILD" TARGET="$TARGET" NO_PROTON="$PROTON_ARG" "$MAKE" "$@" -C ./depends/ V=1 STATIC=1
./autogen.sh

CONFIG_SITE="$PWD/depends/$HOST/share/config.site" CXXFLAGS="-DCURL_STATICLIB -DCURVE_ALT_BN128 -fopenmp -pthread"  ./configure --build="${BUILD}" --host="${HOST}" --target="${TARGET}" --disable-shared --enable-static 
